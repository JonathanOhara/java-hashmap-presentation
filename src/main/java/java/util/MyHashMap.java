...
import static java.util.HashMap.Node;
...
public class MyHashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
        Node<K,V>[] tab; Node<K,V> p; int tableSize, index;
        {
            if ((tab = table) == null || (tableSize = tab.length) == 0)
                tableSize = (tab = resize()).length;
        } //Initialize if the table is null or empty
        if ((p = tab[index = (tableSize - 1) & hash]) == null) //Craziness that I will explain soon (Creating bucket)...
            ...
            if (e != null) {
                ...
                if (!onlyIfAbsent || oldValue == null)
                    e.value = value;
                afterNodeAccess(e); //Callback to override
                return oldValue;
        ...
        if (++size > threshold)
            resize();
        afterNodeInsertion(evict); //Callback to override
        return null;
    }
